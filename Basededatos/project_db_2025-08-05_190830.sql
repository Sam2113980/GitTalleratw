...
BEGIN
    ...
    SELECT
        ...
        a.last_name
    FROM publication p
    INNER JOIN book b ON p.id = b.publication_id
...
/*!50003 SET sql_mode              = 'IGNORE_SPACE,NO_ZERO_IN_DATE,NO_ZERO_DATE,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_delete_book`(IN p_if INT)
BEGIN
    -- Manejador de errores: si algo falla, se revierte la transacción
    ...
    START TRANSACTION;
        DELETE FROM book WHERE publication_id=p.id;
        DELETE FROM publication WHERE id=p.id;
    COMMIT;
...
-- Dump completed on 2025-08-05 19:09:14
